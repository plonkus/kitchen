...
class TestCmdSetupSkillInstall:
    """The skill is installed by copy. A symlink pointed into a working tree and
    died on the next branch checkout, and the old path-identity check called a
    dangling link installed — Path.resolve() is non-strict, so it matched."""
    ...
    def _prep(self, tmp_path):
        ...
        sp = claude_dir / "plugins" / "cache" / "superpowers-marketplace" / "superpowers"
        sp.mkdir(parents=True)
        return claude_dir / "skills" / "claude-kitchen"

    def _run(self, mock_run, monkeypatch, tmp_path, capsys):
        mock_run.return_value = MagicMock(returncode=0, stdout="2.1.99 (claude)\n", stderr="")
        monkeypatch.setenv("HOME", str(tmp_path))
        from claude_kitchen.cli import cmd_setup
        cmd_setup(MagicMock())
    ...
    @patch("claude_kitchen.cli.subprocess.run")
    def test_installs_when_absent(self, mock_run, monkeypatch, tmp_path, capsys):
        target = self._prep(tmp_path)
        assert "✅ Skill installed (copied)" in self._run(mock_run, monkeypatch, tmp_path, capsys)
        assert (target / "SKILL.md").is_file()
        assert not target.is_symlink()

    @patch("claude_kitchen.cli.subprocess.run")
    def test_dangling_symlink_is_repaired(self, mock_run, monkeypatch, tmp_path, capsys):
        """Red on the old code: it printed '✅ Skill installed' and repaired nothing."""
        target = self._prep(tmp_path)
        target.parent.mkdir(parents=True)
        target.symlink_to(tmp_path / "gone" / "skill")
    ...
    @patch("claude_kitchen.cli.subprocess.run")
    def test_live_symlink_is_converted_to_a_copy(self, mock_run, monkeypatch, tmp_path, capsys):
        """A link that resolves today still breaks on the next checkout."""
        target = self._prep(tmp_path)
        source = tmp_path / "checkout" / "skill"
        source.mkdir(parents=True)
        (source / "SKILL.md").write_text("whatever the working tree holds\n")
        target.parent.mkdir(parents=True)
        target.symlink_to(source)
        self._run(mock_run, monkeypatch, tmp_path, capsys)
        assert not target.is_symlink()
        assert (source / "SKILL.md").read_text() == "whatever the working tree holds\n"  # source untouched

    @patch("claude_kitchen.cli.subprocess.run")
    def test_stale_copy_is_refreshed_and_fresh_copy_left_alone(self, mock_run, monkeypatch, tmp_path, capsys):
        target = self._prep(tmp_path)
        target.mkdir(parents=True)
        (target / "SKILL.md").write_text("a drifted older version\n")
        assert "✅ Skill installed (copied)" in self._run(mock_run, monkeypatch, tmp_path, capsys)
        shipped = (target / "SKILL.md").read_text()
        assert shipped != "a drifted older version\n"
        out = self._run(mock_run, monkeypatch, tmp_path, capsys)  # second run: already current
